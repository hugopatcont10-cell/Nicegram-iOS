...
public struct ChatSendMessageContextNicegramData {
    ...
    let translate: () -> Void
    let chooseLanguage: () -> Void
    
    public init(canTranslate: Bool, translate: @escaping () -> Void, chooseLanguage: @escaping () -> Void) {
        self.canTranslate = canTranslate
        self.translate = translate
        self.chooseLanguage = chooseLanguage
    }
    
    public static var empty: ChatSendMessageContextNicegramData {
        ChatSendMessageContextNicegramData(
            canTranslate: false,
            translate: {},
            chooseLanguage: {}
        )
            ...
            if component.nicegramData.canTranslate {
                ...
                items.append(.action(ContextMenuActionItem(
                    ...
                    icon: { theme in
                        ...
                        component.nicegramData.translate()
                        self.environment?.controller()?.dismiss()
                    }
                )))
                
                let title: String
                if let interlocutorLangCode = component.interlocutorLangCode {
                    title = l("Messages.ToLanguage.WithCode", with: interlocutorLangCode.uppercased())
                } else {
                    title = l("Messages.ToLanguage")
                }
                items.append(.action(ContextMenuActionItem(
                    id: AnyHashable("toLanguage"),
                    text: title,
                    icon: { _ in nil },
                    action: { [weak self] _, _ in
                        guard let self, let component = self.component else {
                        ...
                        component.nicegramData.chooseLanguage()
                        self.environment?.controller()?.dismiss()
                    }
                )))
            }
